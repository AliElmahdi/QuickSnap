    ...
    public class SnapGame
    {
		...
		private static void HandleUserInput(Snap myGame)
		{
			...
			if(myGame.IsStarted){
				...
				{
					SwinGame.LoadSoundEffectNamed("Correct", "Correct.wav");
					SwinGame.PlaySoundEffect ("Correct");
				}
		...
		{
			SwinGame.DrawBitmap("PTask2_cardsBoard.png", 0,0);
			...
			if (top != null)
			{
				...
				SwinGame.DrawText ("Top Card is " + top.ToString (), Color.White,"GameFont", 0, 10);
				SwinGame.DrawText ("Player 1 score: " + myGame.Score(0), Color.White, "GameFont", 0, 30);
				SwinGame.DrawText ("Player 2 score: " + myGame.Score(1), Color.White, "GameFont", 0, 50);
